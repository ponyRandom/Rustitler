...
- [ ] PK-01 收敛 Tauri 权限配置，移除无关网络能力（依赖：CO-01 至 CO-15）
- [ ] PK-02 审查 Rust 依赖，确认核心模块不引入网络客户端（依赖：DS-18）
- [ ] PK-03 审查前端依赖，确认运行时不依赖联网服务（依赖：UI-01 至 UI-28）
...
- [ ] PK-08 配置 OCR 语言包运行时路径解析（依赖：PK-05, PK-07, EX-11）
- [ ] PK-09 配置 `.doc` 转换组件随包分发（依赖：DS-08 至 DS-10）
...
- [ ] PK-15 记录 Tesseract、语言包、`.doc` 转换组件和核心依赖许可清单（依赖：PK-02, PK-09）
