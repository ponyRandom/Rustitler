...
| --- | --- | --- |
| Tesseract OCR engine / `tesseract-rs` | Static OCR runtime linked by Cargo | Tesseract is Apache-2.0. `tesseract-rs` is MIT and builds Tesseract/Leptonica at build time; include crate and upstream native library notices in release notices. |
| `tessdata_fast/chi_sim.traineddata` | Bundled Simplified Chinese OCR language data | Official Tesseract traineddata asset. Include the upstream tessdata_fast license/notice in release bundles. |
...
| `image` | PNG/JPEG decode before OCR | Cargo dependency used only with `extraction-ocr`; include crate license and transitive codec notices in generated Cargo notices. |
...
- Copy upstream Tesseract/Leptonica/tessdata license files into the app's third-party notices.
